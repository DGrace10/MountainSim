...
bool SkyPlaneClass::Initialize(ID3D11Device* device, char* cloudTextureFilename, char* perturbTextureFilename)
{
...
bool SkyPlaneClass::LoadTextures(ID3D11Device* device, char* textureFilename1, char* textureFilename2)
{
	bool result;
	...
	result = m_CloudTexture->Initialize(device, NULL, textureFilename1);
	if(!result)
	{
		return false;
	}
	...
	result = m_PerturbTexture->Initialize(device, NULL, textureFilename2);
	if(!result)
	{
		return false;
	}
